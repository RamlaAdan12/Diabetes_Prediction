...
# Dashboard
    ...
    with tabs[0]:
        ...
        if st.button("Predict Diabetes Risk", type="primary", use_container_width=True):
            ...
            # Ensure correct column order & numeric type
            columns_for_model = ['Pregnancies', 'Glucose', 'BloodPressure', 'SkinThickness',
                                 'Insulin', 'BMI', 'DiabetesPedigreeFunction', 'Age',
                                 'RiskLevel', 'BMICategory']

            input_df_model = input_df[columns_for_model].astype(float)

            # Scale input
            try:
                input_scaled = scaler.transform(input_df_model)
            ...
            try:
                ...
                st.error(f"Prediction failed due to model input mismatch: {e}")
                st.stop()
            except Exception as e:
...
with tabs[2]:
    ...
    if st.session_state.prediction_done:
        ...
        fig1.update_layout(title="Prediction Probability", yaxis=dict(range=[0,1], title="Probability"), height=400)
